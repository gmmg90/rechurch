...
import {
  Droplets, Cookie, Star, Heart, TrendingUp, Search, X,
  ArrowRight, CalendarRange, Users,
} from 'lucide-react'
import {
...
import {
  statsApi, battesimiApi, comunioniApi, cresimeApi, matrimoniApi, animeApi,
  type StatsPeriod, type Battesimo, type Comunione, type Cresima, type Matrimonio, type AnimaListItem,
} from '../api/client'
import { formatDate } from '../utils/date'
import AnniversariMatrimoni from '../components/AnniversariMatrimoni'
...
function GlobalSearch() {
  ...
  const { data: ra, isFetching: la } = useQuery({
    queryKey: ['gs-a', dq], enabled,
    queryFn: () => animeApi.list({ search: dq, limit: 5 }),
  })
  const { data: rb, isFetching: lb } = useQuery({
    queryKey: ['gs-b', dq], enabled,
    queryFn: () => battesimiApi.list({ search: dq, limit: 5 }),
  })
  const { data: rco, isFetching: lco } = useQuery({
    queryKey: ['gs-co', dq], enabled,
    queryFn: () => comunioniApi.list({ search: dq, limit: 5 }),
  })
  ...
  const loading = la || lb || lco || lc || lm
  const total = (ra?.items.length ?? 0) + (rb?.length ?? 0) + (rco?.length ?? 0) + (rc?.length ?? 0) + (rm?.length ?? 0)
    ...
    <div ref={ref} className="relative w-full max-w-2xl">
      ...
      {showDrop && (
        <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-100 rounded-xl shadow-xl z-50 overflow-hidden max-h-96 overflow-y-auto">
          ...
          {(ra?.items.length ?? 0) > 0 && (
            <>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 pt-3 pb-1">Anime</p>
              {ra!.items.map((a: AnimaListItem) => (
                <ResultRow
                  key={`a-${a.id}`}
                  onClick={() => { navigate(`/anime/${a.id}`); clear() }}
                  badge="Anima"
                  badgeColor="bg-violet-100 text-violet-700"
                  title={`${a.cognome} ${a.nome}`}
                  sub={a.data_nascita ? formatDate(a.data_nascita) : `${a.total_sacramenti} sacr.`}
                />
              ))}
            </>
          )}
          ...
          {(rb?.length ?? 0) > 0 && (
            <>
              ...
              {rb!.map((b: Battesimo) => (
                <ResultRow
                  key={`b-${b.id}`}
                  onClick={() => { navigate(`/battesimi/${b.id}/certificato`); clear() }}
                  badge="Battesimo"
                  badgeColor="bg-blue-100 text-blue-700"
                  title={`${b.nome} ${b.cognome}`}
                  sub={formatDate(b.data_battesimo)}
                />
              ))}
            </>
          )}
          ...
          {(rco?.length ?? 0) > 0 && (
            <>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 pt-3 pb-1">Comunioni</p>
              {rco!.map((co: Comunione) => (
                <ResultRow
                  key={`co-${co.id}`}
                  onClick={() => { navigate(`/comunioni/${co.id}/certificato`); clear() }}
                  badge="Comunione"
                  badgeColor="bg-emerald-100 text-emerald-700"
                  title={`${co.nome} ${co.cognome}`}
                  sub={formatDate(co.data_comunione)}
                />
              ))}
            </>
          )}
          ...
          {(rc?.length ?? 0) > 0 && (
            <>
              ...
              {rc!.map((c: Cresima) => (
                <ResultRow
                  key={`c-${c.id}`}
                  onClick={() => { navigate(`/cresime/${c.id}/certificato`); clear() }}
                  badge="Cresima"
                  badgeColor="bg-amber-100 text-amber-700"
                  title={`${c.nome} ${c.cognome}`}
                  sub={formatDate(c.data_cresima)}
                />
              ))}
            </>
          )}
          ...
          {(rm?.length ?? 0) > 0 && (
            <>
              ...
              {rm!.map((m: Matrimonio) => (
                <ResultRow
                  key={`m-${m.id}`}
                  onClick={() => { navigate(`/matrimoni/${m.id}/certificato`); clear() }}
                  badge="Matrimonio"
                  badgeColor="bg-rose-100 text-rose-700"
                  title={`${m.sposo_nome} ${m.sposo_cognome} + ${m.sposa_nome} ${m.sposa_cognome}`}
                  sub={formatDate(m.data_matrimonio)}
                />
              ))}
            </>
          )}
          ...
          {total > 0 && (
            <div className="border-t border-gray-50 px-4 py-2.5 flex flex-wrap gap-3">
              ...
              <button onMouseDown={() => { navigate(`/comunioni?search=${dq}`); clear() }}
                className="text-xs text-emerald-500 hover:underline">Tutte le comunioni →</button>
...
export default function Dashboard() {
  ...
  const total = (stats?.battesimi ?? 0) + (stats?.comunioni ?? 0) + (stats?.cresime ?? 0) + (stats?.matrimoni ?? 0)

  const { data: animeTotal } = useQuery({
    queryKey: ['anime-count'],
    queryFn: () => animeApi.list({ limit: 1 }).then(r => r.total),
  })
    ...
    <div className="p-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-6 mb-8">
          <StatCard label="Anime"     value={animeTotal ?? 0}        icon={Users}    color="bg-violet-500"  to="/anime" />
          <StatCard label="Battesimi" value={stats?.battesimi ?? 0} icon={Droplets} color="bg-blue-500"    to="/battesimi" />
          <StatCard label="Comunioni" value={stats?.comunioni ?? 0} icon={Cookie}   color="bg-emerald-500" to="/comunioni" />
          ...
          {donutData.length === 0 ? (
            ...
            <>
              <ResponsiveContainer width="100%" height={210}>
                <PieChart>
                  <Pie {...({ activeIndex: activeDonut, activeShape: ActiveShape } as any)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Aggiungi Comunione', to: '/comunioni/nuovo', color: 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100', icon: Cookie },
