...
import { Search, Plus, Pencil, Trash2, FileDown, Wheat } from 'lucide-react'
...
export default function ComunioniList() {
  ...
  useEffect(() => { document.title = 'Comunioni — ReChurch' }, [])
  useEffect(() => { setPage(1) }, [search, anno])

  const { data: result, isLoading } = useQuery({
    queryKey: ['comunioni', search, anno, page],
    queryFn: () => comunioniApi.listWithTotal({
  ...
  const del = useMutation({
    mutationFn: (c: Comunione) => comunioniApi.delete(c.id),
    onSuccess: (_data, c) => {
      qc.invalidateQueries({ queryKey: ['comunioni'] })
      toast.success(`Comunione di ${c.nome} ${c.cognome} eliminata`)
    },
    onError: () => toast.error('Errore durante l\'eliminazione'),
  })

  const handleDelete = (c: Comunione) => {
    if (confirm(`Eliminare la comunione di ${c.nome} ${c.cognome}?`)) del.mutate(c)
  }
                ...
                {data.length === 0 ? (
                    ...
                    <td colSpan={6} className="text-center py-16">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        {hasSearch
                          ...
                          : <Link to="/comunioni/nuovo" className="text-indigo-600 text-sm hover:underline">Aggiungi il primo record</Link>
                        }
                      </div>
                    </td>
                  </tr>
                  ...
                  data.map(c => (
                    <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-600">{c.data_comunione}</td>
